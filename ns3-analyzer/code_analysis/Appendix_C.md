...
* 위에 기술된 정보 1의 논리는 틀렸음. 1.084516s 시점에 전송한 A-mpdu는 초기 프레임이 아니므로, 계산을 할거면 (BA Req <-> A-mpdu에 대한 BA) 시간을 계산해야됨
  * 따라서, 1.088461s - 1.084382s = 4.079ms 가 맞음, 그렇다고 해도 해석 2의 관점에 가깝기는 함

* 해석 1과 해석 2에 대한 생각
  * TXOP limit은 일반적으로는 해석 1에 가까워야 되는게 맞음
  * 근데, 100% 보장이 안됨, 아래와 같은 이유가 있음
    * 수신기의 위치는 constant한 상태가 아님. 따라서, 지연 시간은 유동적으로 변할 수 있음
